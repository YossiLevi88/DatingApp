...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            users = users.Where(u => u.Id != userParams.Id && u.Gender == userParams.Gender);

            if (userParams.Likers)
            {
                var likers = await GetUserLikes(userParams.Id, userParams.Likers);
                users = users.Where(u => likers.Contains(u.Id));
            }

            if (userParams.Likees)
            {
                var likees = await GetUserLikes(userParams.Id, userParams.Likers);
        ...
        {
            var user = await _context.Users.Include(u => u.Likees).Include(u => u.Likers)
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParamas)
        {
            ...
            {
                case "Inbox":
                    messages = messages.Where(m => m.RecipientId == messageParamas.UserId && !m.RecipientDeleted);
                    break;
                case "Outbox":
                    messages = messages.Where(m => m.SenderId == messageParamas.UserId && !m.SenderDeleted);
                    break;
                default:
                    messages = messages.Where(m => m.RecipientId == messageParamas.UserId && !m.RecipientDeleted && !m.IsRead);
                    break;
            }
